...
exports.menu = async (req, res, next) => {
    ...
    // const menu = await Menu.find().select('-shop').where('price').gt(100).sort('-price');
    // const menu = await Menu.find({ price: {$gte: 200 } });
    const menu = await Menu.find().populate('shop', 'name location') // many to one
    res.status(200).json({
        data: menu
    })
}
...
exports.getShopWithMenu = async (req, res, next) => {
    const { id } = req.params;
    const shopWithMenu = await Shop.findById(id).populate('menus') // one to many
    res.status(200).json({
        data: shopWithMenu
    })
}
